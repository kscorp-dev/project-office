# Project Office v0.23.6

사내 업무 통합 플랫폼 - 전자결재, 메신저, CCTV, 근태관리, 작업지시서 외 12개 모듈

> **Production**: [https://43-200-29-148.sslip.io](https://43-200-29-148.sslip.io)
> **운영 안정성 (Production Readiness) 보강 — Readiness probe / Graceful shutdown / CSP / Audit log 통일**<br/>
> **단일 인스턴스 운영 안전성 확보** ✅ (Multi-instance scale-out 가이드 docs/ 추가)
...
```bash
# Backend (330 유닛/통합 테스트 — 일부 통합 테스트는 PostgreSQL 필요)
cd apps/backend
npm test                    # 전체 실행
...
| 버전 | 주요 변경 |
|------|----------|
| **v0.23.6** | **운영 안정성 보강 (Production Readiness) — readiness probe / graceful shutdown / CSP / 감사 로그 통일**<br/>v0.23.5 의 12차 검증 후 보안 결함은 0 인 상태에서, 실제 운영에서 문제 될 만한 production readiness 영역으로 확장. 두 audit agent 병렬 → 운영 위험 30+건 식별 → 단일 인스턴스에서 즉시 영향 있는 17건 수정 + 다중 인스턴스 보강은 docs 가이드.<br/>**[Critical 운영 — 단일 인스턴스 즉시 영향]**<br/>• **/ready (readiness probe) 신규** — DB + Redis ping 으로 503 응답 가능. K8s readinessProbe 또는 LB 트래픽 라우팅 결정용. /health 는 liveness (단순 alive).<br/>• **Graceful shutdown 강화** — httpServer.close + io.disconnectSockets/close + 워커 정리 + prisma.$disconnect 순차 실행, 25s force-kill fallback (K8s grace 30s 안). unhandledRejection / uncaughtException 핸들러 추가 (silent crash 방지).<br/>**[High 운영]**<br/>• **CCTV audit log 보강** — camera_create/update/delete/permission_grant/revoke/ptz_command/stream_start/stop 8개 enum + Prisma migration. 모든 mutation 라우트 createAuditLog 호출 (compliance).<br/>• **findMany take 추가** — cctv (1000) / cameraGroup (200) / calendar event (2000) — 무제한 fetch 메모리 폭발 방지.<br/>• **Vacation.days 검증** — 1~31일 한도 + calendar 일수 일관성 (반차 0.5 = 1일, 일반 days <= calendar). 클라이언트 부풀린 days 로 잔여 부정 차단.<br/>• **prisma.$transaction timeout 명시** — createDocument 가 timeout 15s + maxWait 5s. 휴가 양식 + 결재선 + 참조자 + Vacation 동시 생성에 default 5s 부족.<br/>• **audit log X-Forwarded-For → req.ip** — 직접 header read 는 spoofing 가능. trust proxy 의존.<br/>• **audit log 실패 logger.error CRITICAL** — 이전 console.error 는 unstructured (CloudWatch/Loki 파싱 깨짐).<br/>• **CSP (Content Security Policy)** — defaultSrc 'none' / scriptSrc 'self' / frameAncestors 'self' / 메일 인라인 style 허용.<br/>• **JWT secret 검증 모든 환경** — isProd 한정 → 모든 환경 32자 강제 + dev fallback 32자 보장.<br/>**[Medium]**<br/>• calendar PATCH 시 partial 검증<br/>• vacation 회귀 테스트 2건 (INVALID_DAYS / DAYS_MISMATCH)<br/>**[Multi-instance scale-out — 가이드 문서로 정리]**<br/>• docs/Multi-Instance-Scale-Out.md (NEW) — 단일 인스턴스 → N개 확장 시 보강 절차<br/>  - rate-limit-redis store 도입<br/>  - @socket.io/redis-adapter + sticky session<br/>  - cron leader election (pg_advisory_lock)<br/>  - 검증 절차 + 견적 (총 2.5일 작업)<br/>• 단일 인스턴스 운영에선 즉시 위험 없음 — scale-out 결정 시점에 진행<br/>**[검증]**<br/>• 백엔드 328 → **330 테스트** 통과 (+2 vacation validation 회귀)<br/>• Prisma migration 신규 1건 (CCTV audit actions 8개)<br/>• TypeScript backend/mobile 깨끗, mobile verify:build 45/45<br/>• graceful shutdown / readiness probe / CSP / audit 일관 — production 단일 인스턴스 안전성 확보 |
| v0.23.5 | **10-12차 감사 — 비즈니스 로직 무결성 + 인프라 (18종 수정)**<br/>v0.23.4 의 9차 검증 후 새로운 영역(비즈니스 로직 race / 인프라 / 위임 cycle) 으로 10차 확장 감사. 두 audit agent 병렬로 30+ 후보 식별 → Critical/High 18종 수정 → 11차 verify 에서 추가 3건 발견 → 즉시 수정 → 12차 verify 통과 (Critical/High 0).<br/>**[Critical 4종]**<br/>• **C1 결재선 자기 자신 / 중복** — drafter 가 approverIds 에 포함되면 자기 결재 우회. 같은 결재자 중복 시 두 번째 라인 영원히 stuck. createDocument 검증.<br/>• **C2 휴가 잔여 race** — 신청 시점 검증만 있어 동시 두 건 승인 시 음수 가능. applyVacationOnFinalApproval 트랜잭션 안 잔여 재검증 + 자동 반려.<br/>• **C3 재고 음수 race** — currentStock 동시 update race. POST /transactions 내부 SELECT FOR UPDATE row lock + raw 값으로 산술.<br/>• **C4 위임 통한 자기 결재 우회** — createDocument 의 SELF_APPROVAL 검증을 B→A 위임으로 우회. approve/reject 에 `viaDelegation && approverId === doc.drafterId` 차단 (11차 발견).<br/>**[High 9종]**<br/>• 메신저 1:1 자기 자신 채팅 차단<br/>• 메시지 parentId 검증 (다른 룸/삭제된 메시지)<br/>• mentionIds 50명 한도<br/>• 캘린더 endDate < startDate / repeatUntil < startDate (POST + PATCH)<br/>• 디자인 파일 자기 승인 차단<br/>• 회의 ring 상태(ended/cancelled) 검증<br/>• WebSocket maxHttpBufferSize 256KB (DoS 차단)<br/>• urlencoded extended:false + parameterLimit (prototype pollution)<br/>• User PATCH `.strict()` (employeeId/password 등 unknown 거부)<br/>• 위임 transitive cycle (A→B + B→A) 검증<br/>• 위임 reverseChain race — advisory lock 키 페어 정렬 (11차 발견)<br/>**[Medium 5종]**<br/>• 위임 create/cancel audit log (delegation_create/cancel enum 추가 + migration)<br/>• inventory unitPrice nonnegative + quantity 1M max<br/>• attendees 200명 max<br/>• mentions 50명 max<br/>**[12차 독립 검증]**<br/>• Self-approval via delegation: createDocument(직접) + approve/reject(위임 path) 양쪽 차단 OK<br/>• Calendar PATCH 시간 검증: finalStart/finalEnd/repeatUntil 일관성 OK<br/>• Delegation lock 페어 정렬: A→B / B→A 동시 호출 시 동일 lock → snapshot 일관성 OK<br/>• **Critical Found: None / High Found: None — 최종**<br/>**[검증]**<br/>• 백엔드 **328 테스트** 통과 (변동 없음)<br/>• Prisma migration 신규 (delegation_create/cancel audit actions)<br/>• TypeScript backend/mobile 깨끗, mobile verify:build 45/45<br/>• 1~12차 누적: **61건 결함 식별 + 모두 수정** |
| v0.23.4 | **7-9차 종합 감사 — IDOR 차단 + 운영 보안 + 모바일 메모리 누수 (15종 수정)**<br/>v0.23.3 의 6차 검증 후 새로운 영역(운영/배포 / 데이터 격리 / Dockerfile / OAuth state) 으로 7차 확장 감사. 발견된 15종 결함 모두 수정하고 8차/9차 독립 검증으로 Critical/High 0건 최종 확인.<br/>**[Critical 3종 — 데이터 노출/탈취]**<br/>• **C1 `/uploads` 무인증 정적 노출** — 메신저 첨부, 회의 자료 등 모든 업로드 파일이 인증 없이 서빙됨. UUID 파일명만 추측하면 누구든 다운로드 가능. server.ts 에 인증 미들웨어 (Bearer + ?token= 쿼리스트링 fallback) + mobile absUrl() 자동 token 첨부.<br/>• **C2 Google OAuth state CSRF — 계정 탈취** — `state=<userId>` 평문 신뢰. 공격자가 본인 Google code + 피해자 userId 콜백 → 피해자 계정에 본인 토큰 저장. signState/verifyState (HMAC-SHA256 + nonce + 10분 TTL + timing-safe).<br/>• **C3 회의 GET /:id leftAt 누설** — 과거에 leave 한 참여자 정보 노출. 응답에서 활성 참여자만 필터 (admin 은 운영 목적 전체 조회).<br/>**[High 3종 — race / SSRF / root]**<br/>• **H1 결재 회수(withdraw) race** — find→update 사이 동시 approve race. transaction + updateMany 조건부 (`status='pending' AND lines.every({status:'pending'})`).<br/>• **H2 CCTV rtspUrl SSRF** — `z.string().min(1)` 만 검증 → ffmpeg 에 임의 URL 전달. regex `^rtsps?://(?:user:pass@)?host(:port)?/path$` 강제. user:pass@host 호환.<br/>• **H3 Dockerfile root 실행** — apps/web (nginx) + apps/detection (uvicorn) 모두 root. USER nginx (port 8080) / USER appuser 추가, docker-compose port mapping 8080:8080.<br/>**[Medium 3종 + IDOR 4종]**<br/>• 비밀번호 정책 일관성 — invite/reset 도 영문+숫자+특수문자 regex 강제 (validatePasswordStrength)<br/>• callkeep uuidToMeeting Map 메모리 누수 — 5분 TTL + 50개 cap + clearPendingCalls() (사용자 변경 시)<br/>• usePushNotifications recentRingUuids Map 누수 — 5분 TTL<br/>• **Task-orders IDOR** (직접 발견) — GET /:id / POST comments / PATCH checklist 모두 권한 체크 없었음. canAccessTask 적용 + cross-task checkId 검증<br/>**[8-9차 회귀 검증]**<br/>• docker-compose.staging port 80 → 8080 (web Dockerfile 변경 따라)<br/>• RTSP regex 가 `user:pass@host` 거부했던 회귀 — `(?:[^@\s/]+@)?` 그룹 추가<br/>**[검증]**<br/>• 백엔드 317 → **328 테스트** 통과 (+11 IDOR 회귀)<br/>• TypeScript backend/mobile 깨끗, mobile verify:build 45/45<br/>• 9차 독립 감사: **Critical 0 / High 0** ← 최종<br/>• 1~9차 누적: 43건 결함 식별 + 모두 수정 |
| v0.23.3 | **4-6차 종합 감사 + 추가 결함 8종 수정 — Mass Assignment / Rate Limit / 추가 sensitive 화면**<br/>v0.23.2 의 3차 감사 후 더 새로운 영역(file upload / PATCH mass assignment / rate limit / deep link / WebSocket 다른 namespace) 으로 확장 감사. 발견된 8종을 수정하고 6차 독립 검증으로 Critical/High 0건 최종 확인.<br/>**[Critical 3종 — PATCH mass assignment]**<br/>• **C1 Calendar PATCH scope 격상** — 일반 사용자가 본인 personal 일정의 scope 를 'company' 로 격상해 회사 전체 캘린더에 spam 가능. POST/PATCH 양쪽에 admin/dept_admin 권한 체크 + 화이트리스트 destructure (creatorId/id/createdAt 차단).<br/>• **C2 Task-orders PATCH status 우회** — `req.body` spread 로 status='final_complete' 직접 set 가능 → STATUS_TRANSITIONS 검증 우회. status/completedAt/creatorId/taskNumber 화이트리스트 제외 (상태 변경은 POST /:id/status 단일 경로).<br/>• **C3 Meeting PATCH** — 호스트가 hostId 변경(소유권 탈취) / roomCode 변경(unique 충돌) / status='ended' 강제 가능. title/description/maxParticipants/password/scheduledAt 화이트리스트로 한정.<br/>**[High 5종]**<br/>• **H1 Inventory/Parking/CCTV PATCH** — `data: req.body` 그대로 전달 → validate(*Schema.partial()) 적용으로 zod 검증 강제.<br/>• **H4 Calendar departmentId 변경** — dept_admin 이 본인 부서 → 타 부서 일정 reassign 가능. admin/dept_admin 권한 체크 추가.<br/>• **H5 /admin/push/test 스팸** — 어드민 탈취 시 단말 푸시 폭격. 분당 5회 rate limit (test skip).<br/>• **H6 /meeting/:id/ring 스팸** — 호스트가 분당 ~100회 ring → 참가자 N명에게 push N×100 폭격. 분당 6회 rate limit.<br/>• **H7 /messenger/rooms/:id/members 대량 초대** — 분당 50×100=5000명 초대 가능. 분당 5회 rate limit.<br/>**[Mobile 추가 보호]**<br/>• 추가 sensitive 화면 3종에 useScreenCaptureBlock — notifications.tsx (알림 본문에 결재/메일 일부) / task-orders/[id].tsx (영업/단가/거래처) / settings/delegation.tsx (위임자 개인정보)<br/>• verify-build.js 의 sensitive screens 3 → 6 으로 확장<br/>**[6차 독립 감사 결과]**<br/>• rate limiter test skip 검증: production 영향 없음 (NODE_ENV !== 'test')<br/>• PATCH 화이트리스트 destructure 패턴 = fail-closed (새 필드 도입 시 명시 필요)<br/>• calendar departmentId 권한 edge case (admin no-op / dept_admin 본인 부서) 모두 OK<br/>• **Critical Found: None / High Found: None — 최종 검증 통과**<br/>**[검증]**<br/>• 백엔드 313 → **317 테스트** 통과 (+4 회귀: scope 격상 차단 / 부서 일정 정책 / hostId 차단 / creatorId 차단)<br/>• TypeScript backend/mobile 깨끗, mobile verify:build 45/45 통과<br/>• 1차~6차 누적 28건 결함 모두 수정 |
| v0.23.2 | **2차 종합 감사 + 추가 결함 13종 수정 (3차 검증 Critical 0, High 0 확인)**<br/>v0.23.1 의 1차 감사 후 더 깊은 영역(WebSocket / 위임 권한 path / 동시성)으로 확장 감사. 발견된 13개 결함을 모두 수정하고 독립 감사 에이전트로 3차 검증해 Critical/High 잔재 0건 확인.<br/>**[2차 감사 Critical 6종]**<br/>• **C1 WebSocket meeting events 룸 미검증** — `meeting:offer/answer/ice-candidate` 가 `data.to` socket ID 검증 없이 broadcast → 다른 회의 참가자 RTC 상태 오염 가능. `senderInRoom()` / `targetInSameRoom()` 헬퍼로 일관 검증.<br/>• **C2 `/api/meeting/:id/decline` 참가자 미검증** — 인증된 누구나 임의 회의에 호출 가능 → 호스트 스팸. `meeting.participants` 또는 `hostId` 검증 추가.<br/>• **C3 WebSocket messenger typing/read 룸 미검증** — 비참가자가 타이핑/읽음 이벤트 주입 가능. typing 60s TTL 멤버십 캐시 + `message:read` 는 `updateMany + leftAt:null` 명시.<br/>• **C4 결재 detail 위임 권한 누락** — 위임받은 사용자가 결재 도착 푸시 탭 시 403 (처리만 가능, 화면 못 봄). `getDocumentDetail` 에 `findFirst(approvalDelegation)` 추가.<br/>• **C5 결재 대기 목록 위임 누락** — 모바일 결재 탭에서 위임받은 결재가 0건 표시. `getDocuments(box=pending)` 에 `delegators` 합산 + `approved` 박스에서 `actedByUserId` 매칭.<br/>• **C6 결재 카운트/첨부 위임 누락** — `/count` 와 `canAccessAttachment` 모두 위임자 결재 합산/포함.<br/>**[2차 감사 High 4종]**<br/>• **H1 `.env.example` AWS Access Key ID 평문 노출** — 주석 내 실제 형식 키 ID. 마스킹 처리 + IAM 키 rotate (운영 액션).<br/>• **H2 모바일 refresh token 동시 갱신 race** — 여러 401 요청이 각자 refresh → 첫 요청만 성공, 나머지 강제 logout. module-level `refreshPromise` 캐시.<br/>• **H3 위임 생성 race** — 동시 POST 두 개가 둘 다 overlap 검사 통과해 둘 다 create. `prisma.$transaction` + `pg_advisory_xact_lock(hashtext("from:to"))` 직렬화.<br/>• **H4 `canActOnLine` 트랜잭션 격리 위반** — approve/reject tx 안에서 위임 조회를 전역 prisma 로 → stale read. tx 옵션 매개변수 추가.<br/>**[추가 발견 + 추가 fix 3종]**<br/>• **fetchMe 캐시 wipe 누락** — 1차 C1 의 잔재. `set({user:null,...})` 가 cache wipe 미호출 → 토큰 만료 자동 logout 시 PII 누출. `get().logout()` 호출로 일관화.<br/>• 외부인 decline 시도 → 403 + 호스트 알림 0건 회귀 테스트<br/>• 위임 detail/list 권한 회귀 테스트 3건<br/>**[3차 독립 감사 결과]**<br/>• tx 전파 / WebSocket 룸 검증 일관성 / advisory lock 안전성 / refreshPromise race / fetchMe-logout 무한루프 / getDocuments 중복 표시 — 모두 안전 확인<br/>• **Critical Found: None / High Found: None**<br/>**[검증]**<br/>• 백엔드 309 → **313 테스트** 통과 (+4 회귀)<br/>• TypeScript backend/mobile 깨끗, mobile verify:build 42/42 |
| v0.23.1 | **종합 감사 + 핵심 결함 7종 수정 (Critical 3 + High 4)**<br/>v0.23.0 시점에 전체 코드베이스를 종합 audit 한 결과 발견된 정합성/보안/권한 결함을 일괄 수정. 백엔드 테스트 305 → 309, 모든 핫패스 회귀 검증.<br/>**[Critical 3종]**<br/>• **C1 캐시 wipe 누락 (PII 누출)** — 로그아웃/계정 변경 시 SQLite 캐시(메시지/메일/캘린더 본문)가 그대로 남아 다음 사용자에게 직전 사용자 데이터가 잠시 노출. `auth.ts logout()` 에 `clearOfflineDb()` 호출, `login()` 도 prev user 와 다른 계정 시 wipe.<br/>• **C2 drizzle expo-sqlite transaction 시그니처 오용** — `db.transaction(async (tx) => {...})` 패턴이 sync API 라 BEGIN/COMMIT 가 callback resolve 전에 닫히고 inner inserts 가 트랜잭션 밖에서 실행 또는 silent fail. P3-A 캐시 의도가 동작하지 않음. 5개 파일(useChatRooms / useMessagesCache / messenger room / calendar / mail)에서 단건 upsert 패턴으로 재작성.<br/>• **C3 대시보드 scope 매핑 불일치** — schema 는 `personal/department/company` 인데 dashboard 가 `all/personal_dept` 로 쿼리해 회사 전사·부서 일정 매칭 0. 사용자가 위임자/참석자가 아닌 일정은 `todayEvents` 카운트 누락. 매핑 정정 + `attendees.status: { not: 'declined' }` 필터 추가.<br/>**[High 4종]**<br/>• **H3 그룹 멤버 추가 시 푸시 알림 미발송** — 오프라인/앱 미실행 멤버가 초대 사실 인지 불가. `createNotification(type='message_received')` 자동 발사.<br/>• **H4 인벤토리 lowStock 페이징 깨짐** — column-vs-column 비교(`currentStock <= minStock`)는 Prisma 미지원으로 후처리 filter 사용했는데 meta.total 은 전체 카운트. raw SQL 로 후보 ID 추출 후 `where.id = { in: [...] }` 적용.<br/>• **H5 그룹 초대 권한 정책 불일치** — DELETE 는 방장만 타인 제거, POST 는 일반 멤버도 누구나 초대 가능. Slack 식 정책 일치(방장/admin 만 초대).<br/>• **H6 위임 중복 생성 차단** — 같은 from→to 사용자에게 기간 겹치는 활성 위임 무한 생성 가능했음. createDelegation 에 겹침 검사 + `409 DUPLICATE_DELEGATION`.<br/>**[H7 회귀 테스트 + M3]**<br/>• 신규 4건: 위임 중복 차단(2) + 그룹 초대 권한(1) + 멤버 추가 알림(1)<br/>• `apps/mobile/prisma/` 빈 디렉토리(stale 잔재) 삭제<br/>**검증**<br/>• 백엔드 309 테스트 통과, TypeScript backend/mobile 깨끗, verify:build 42/42<br/>• 위임/메신저/대시보드/인벤토리 4 도메인 회귀 검증 |
| v0.23.0 | **Phase 3 진행 — 오프라인 SQLite 캐시 + 스크린샷 차단**<br/>모바일 앱이 "네트워크 가능할 때만 동작"에서 "네트워크 끊겨도 직전 데이터 즉시 표시 + 복구 시 자동 갱신"으로 도약. 사내 컨플라이언스(스크린샷 차단)도 포함.<br/>**① 오프라인 SQLite 캐시 (Phase 3-A)**<br/>• 기획 §5.7 + 부록 B 의 SQLite (drizzle + expo-sqlite) 인프라 활성화<br/>• v0.21.0 의 `useChatRooms` (룸 목록) 만 캐시-우선이었음 → 핵심 데이터 3축에 동일 패턴 확산<br/>• 모든 캐시-우선 hook 공통 원칙:<br/>  1. SQLite 캐시 즉시 로드 → setState (UI 깜빡임 방지)<br/>  2. 동시에 GET API 호출, 응답 도착 시 setState + 캐시 upsert (비동기)<br/>  3. API 실패 + 캐시 hit 시 alert 생략 (조용한 폴백)<br/>• 신규/적용:<br/>  - `useMessagesCache.ts` 신규 hook + `messenger/room/[id].tsx` loadInitial 통합<br/>  - `calendar.tsx` fetchEvents — 월 범위 query (gte/lte/and)<br/>  - `(tabs)/mail.tsx` fetchMessages — folder=INBOX 만 캐시 (검색은 서버만)<br/>• 결과: 비행기 모드 / 지하철 / 사무실 와이파이 끊김에서도 직전 데이터 즉시 표시<br/>**② 스크린샷 차단 (Phase 3-B)**<br/>• 기획 §9.3 — 민감 화면 OS 스크린샷/녹화 차단 (사내 기밀 + 컨플라이언스)<br/>• `expo-screen-capture` 의존성 + `useScreenCaptureBlock` hook<br/>  - 마운트 시 preventScreenCaptureAsync, 언마운트 시 allowScreenCaptureAsync<br/>  - 라이브러리 미존재 환경(Expo Go) 안전하게 noop<br/>• 적용 화면 3종:<br/>  - approval/[id].tsx (결재 본문 — 위임/대결 포함)<br/>  - mail/[uid].tsx (메일 본문 + 첨부)<br/>  - messenger/room/[id].tsx (채팅 내용)<br/>• 플랫폼 동작:<br/>  - Android: FLAG_SECURE — 스크린샷/녹화/작업전환 thumbnail 모두 차단<br/>  - iOS: 시스템 차단 미지원, 스크린샷 알림만 (Phase 4 검토)<br/>**③ 검증**<br/>• `verify-build.js` 36 → 42 항목 (스크린샷 차단 4 + offline-db 2 추가)<br/>• TypeScript 깨끗, 백엔드 테스트 305건 그대로 통과 (백엔드 변경 없음) |
...
- [Multi-Instance Scale-Out 가이드](docs/Multi-Instance-Scale-Out.md) — Redis store + Socket.IO adapter + cron leader election
- [Phase 3 잔여 옵션](docs/Phase3-잔여-옵션.md) — Widget(iOS) + App Attest 향후 진행 가이드
